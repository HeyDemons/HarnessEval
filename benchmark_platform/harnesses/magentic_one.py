...
def _magentic_team(_names: set[str]) -> dict[str, str]:
    """Keep the official participant topology independent of benchmark tools.

    The benchmark bridge, not the Magentic-One profile, owns the tool contract.  Removing a
    participant merely because one canonical AutoGen tool is absent changes the routing graph
    and lets benchmark-specific capabilities redefine the algorithm itself.
    """
